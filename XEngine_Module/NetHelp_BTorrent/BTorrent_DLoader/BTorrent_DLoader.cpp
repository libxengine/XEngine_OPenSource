...
XBOOL CBTorrent_DLoader::BTorrent_DLoader_Create(XNETHANDLE* pxhToken, LPCXSTR lpszAddr, LPCXSTR lpszSavePath, LPCXSTR lpszTempFile /* = NULL */)
{
    ...
    if ((NULL == pxhToken) || (NULL == lpszAddr) || (NULL == lpszSavePath))
    {
        ...
        BTDload_dwErrorCode = ERROR_XENGINE_DOWNLOAD_BTORRENT_DLOAD_CREATE_PARAMENT;
        return FALSE;
    }
	//申请空间
    ...
    {
        BTDload_IsErrorOccur = TRUE;
        BTDload_dwErrorCode = ERROR_XENGINE_DOWNLOAD_BTORRENT_DLOAD_CREATE_MALLOC;
        return FALSE;
    }
	//初始化要查询的事件
...
XBOOL CBTorrent_DLoader::BTorrent_DLoader_Query(XNETHANDLE xhToken, ENUM_BTORRENT_EVENT_TYPE*** pppenEventList, int* pInt_ListCount)
{
	...
	{
		BTDload_IsErrorOccur = TRUE;
		BTDload_dwErrorCode = ERROR_XENGINE_DOWNLOAD_BTORRENT_DLOAD_QUERY_PARAMENT;
		return FALSE;
	}
	...
	{
		BTDload_IsErrorOccur = TRUE;
		BTDload_dwErrorCode = ERROR_XENGINE_DOWNLOAD_BTORRENT_DLOAD_QUERY_NOTFOUND;
	...
	{
		BTDload_IsErrorOccur = TRUE;
		BTDload_dwErrorCode = ERROR_XENGINE_DOWNLOAD_BTORRENT_DLOAD_QUERY_NONE;
		st_Locker.unlock_shared();
		return FALSE;
	}
...
XBOOL CBTorrent_DLoader::BTorrent_DLoader_GetStatus(XNETHANDLE xhToken, BTORRENT_DLOADER* pSt_DLStatus)
{
	...
	{
		BTDload_IsErrorOccur = TRUE;
		BTDload_dwErrorCode = ERROR_XENGINE_DOWNLOAD_BTORRENT_DLOAD_GETSTATUS_PARAMENT;
		return FALSE;
	}
	st_Locker.lock_shared();
	unordered_map<XNETHANDLE, BTORRENT_DLOADINFO*>::const_iterator stl_MapIterator = stl_MapDLoader.find(xhToken);
	...
	{
		BTDload_IsErrorOccur = TRUE;
		BTDload_dwErrorCode = ERROR_XENGINE_DOWNLOAD_BTORRENT_DLOAD_GETSTATUS_NOTFOUND;
		st_Locker.unlock_shared();
		return FALSE;
	}
